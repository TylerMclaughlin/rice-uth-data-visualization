...
Let's manipulate data.tables, with "[i, j, by]" notation.
"i" is the rows,  "j" is the columns, and  you will see what "by" means: 
...
That yielded the mean for x, which is the same for all sets.  The same is also true for the mean for y.  This is *not* a bug!  Four distinct data sets look very different yet they are exactly same **on average**.  
...
Ok, before we move on to another data set, let's compute the Pearson Product-Moment Correlation Coefficient (Person's r):
...
":=" translates roughly to "add *this* new column with *this* definition"
...
To avoid overplotting, let's filter the data, and plot the first 300 data points.  We are filtering rows, so let's do this in data.table "i"
```{r}
...
This looks OK, but it is nothing special. Recall that the figure axes are amino acid numbers. We may want to know the exact residue number, and that is not determinable by eye.  
...
Recall that we already defined a variable with this value, 'last.value'.
It's good programming practice to frequently test that variables are equal.  
...
Great.  Next, we'll color using the ramp.  
...
improving readability without affecting the results.
...
Take a standard R plot and wrap it in a renderPlot() function.  Rbokeh plot in a renderRbokeh().
